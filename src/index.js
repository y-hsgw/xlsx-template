...
class Workbook {
    ...
    sourceRels.root.findall("Relationship").forEach(function (rel, index) {
      ...
      if (needsFileCopy.indexOf(relType) !== -1) {
        ...
        if (sourceFile) {
          ...
          var newFileName =
            baseNameWithoutNumber + newSheetIndex + fileExtension;
          var newTarget = path.join(fileDir, newFileName).replace(/\\/g, "/");
          var newFilePath = path
            .join(sheetDirectory, newTarget)
            .replace(/\\/g, "/");
          ...
          if (
            ...
            var commentsContentType = etree.SubElement(
              self.contentTypes,
              "Override",
            );
            ...
            var threadedCommentContentType = etree.SubElement(
              self.contentTypes,
              "Override",
            );
  ...
  substituteImageInCell(cell, substitution) {
    ...
    const futureMetadata = this.metadata
      .findall("futureMetadata")
      .find(function (fm) {
